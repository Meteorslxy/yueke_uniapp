...
function checkForm(fields, forms, that) {
    ...
    for (let k = 0; k < fields.length; k++) {
        ...
        switch (type) {
            case 'rows': {
                // 多条目的数据校验
                let ret = that.zpSelectComponent('#rows' + k).checkForms();
                if (ret !== true) {
                    return;
                }
                break;
...
function getTypeOptions() {
    ...
    return dataHelper.getSelectOptions(
        'text=单行文本,select=单项选择,checkbox=多项选择,switch=开关选择,carnumber=车牌,textarea=多行文本,idcard=身份证号码,date=日期 (年 月 日),month=月份,year=年份,hourminute=时间点,area=省市区,int=整数数字,digit=带小数点的数字'
    );
}
