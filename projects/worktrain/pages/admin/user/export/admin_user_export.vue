        ...
        <view class="main-admin" v-if="isAdmin && isLoad">
            <view class="form-box shadow">
                <view class="form-group" style="padding: 20rpx 15rpx 20rpx">
                    <view class="title-desc">※ 数据说明： 针对本次查询结果导出全部数据</view>
                </view>
            ...
            <button @tap="bindExportTap" class="btn-admin margin-top">{{ url ? '重新生成数据' : '生成数据' }} (Excel文件)</button>

            <button v-if="url" @tap="bindOpenTap" class="btn-admin bg-purple light">直接打开数据文件</button>

            <button v-if="url" @tap="bindDelTap" class="btn-admin margin-bottom-s bg-red light">删除数据文件</button>
